/**
 * Fornece um conjunto de classes para criação de {@link org.springframework.validation.Validator}s
 * para validação de {@link io.github.manoelcampos.vendas.api.model.AbstractBaseModel}.
 * Se a entidade não precisar de regras de validação personalizadas,
 * não é necessário criar uma classe de validação para ela.
 * Ver a documentação de {@link io.github.manoelcampos.vendas.api.shared.validator.CustomValidator}
 * para mais detalhes.
 */
package io.github.manoelcampos.vendas.api.shared.validator;
